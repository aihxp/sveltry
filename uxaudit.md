...
### [PRF-001] Core Web Vitals and layout-shift not verified (runtime check needed) [MEASURED - production Lighthouse; lab CWV good on the public route, authed pages + field data still open]
- Method: Built the dashboard for production (`vite build`, adapter-node) and ran Lighthouse 12.8.2 (headless Chrome 149, `throttlingMethod: "simulate"`, one cold run per form factor) against the production server (`node build/index.js`) on `http://127.0.0.1:5180/`. Simulated throttling: mobile 150 ms RTT / 1638 Kbps / 4x CPU, desktop 40 ms / 10240 Kbps / 1x CPU. These are modeled lab estimates on a fast host (benchmarkIndex ~4249), not measured-device or field data, and a single run carries no variance/median. (These numbers were re-derived from the raw Lighthouse JSON by an independent verification panel.)
- Public landing `/`: Performance **98/100 mobile, 100/100 desktop**; LCP **1.8s mobile (1806.8 ms) / 0.5s desktop (484.2 ms)** (LCP element is the hero `<h1>`); **CLS 0** both; **TBT 0ms** both; Speed Index 1.8s/0.5s; main-thread work 0.2s. Mobile is 98 (not 100) only because `first-contentful-paint` scores 0.89 (1.8s); every other sub-metric passes. On this single simulated run, lab CWV is good for the one measured public route.
- Three perf audits are non-passing, not one: `uses-text-compression` (~17 KiB) and `document-latency-insight` (~14 KiB, "No compression applied") are gzip artifacts of the bare local `node` server and are *expected* (not verified) to clear behind a compressing proxy/CDN; `bf-cache` fails because the page holds a persistent Convex WebSocket ("Pages with WebSocket cannot enter back/forward cache"). The bf-cache miss is real production behavior that a proxy/CDN will NOT fix, and it costs instant back/forward navigation restore, worth a conscious tradeoff decision.
- Bundle (v0.11.0, gzipped): total client JS across all route chunks ~230 KB (sum of all 80 emitted `.js`; no single page transfers this, a page loads only the app shell + shared chunks + its route chunk); per-route page chunks ~0.8-15 KB (heaviest route ~15 KB); app-shell entry (`app.js` + `start.js`) ~2.9 KB.
- Authed app pages (Overview, Issues) were NOT lab-measured on the production build: auth is origin-pinned to `PUBLIC_APP_URL = localhost:5174` and needs valid credentials. On an earlier DEV build, one real-Chrome `PerformanceObserver` session showed CLS 0, no long tasks >50ms, no failed requests, and ~225 DOM nodes; dev builds are unminified, unbundled, and HMR-instrumented, so these are directional structural signals only and do not predict the prod build's throttled CWV.
- Net: lab CWV is good on the one measured public route (simulated, single run); the two compression items are expected to clear behind a CDN, but the WebSocket-driven bf-cache miss persists in production. No field data exists yet (`loadingExperience`/`originLoadingExperience` are absent in both reports), so real-user CWV is unconfirmed. Open items: prod-build authed-page measurement, a decision on the bf-cache tradeoff, and a web-vitals/RUM beacon (Sveltry can dogfood its own perf ingest) as the field source of truth. No public-landing performance code change is required for the compression items.
