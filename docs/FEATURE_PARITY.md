...
| Public REST API / API tokens | Partial | A v1 API authenticated by org API tokens (Bearer, `read` or `read+write` scope, managed on the settings page): `GET /projects`, `/releases`, `/members`, `/projects/<slug>/issues`, `/projects/<slug>/deploys`, `/projects/<slug>/events/<eventId>` (strict), `/issues/<id>`, `/events/<eventId>`, `/issues/<id>/events`, and `POST /issues/<id>/{assign,resolve,ignore,unresolve}` (write scope). List endpoints support opaque cursor pagination (`?cursor`, `?limit`, `nextCursor`). Still missing vs Sentry: an artifacts resource. |
...
| Outbound webhooks | Done | Per-project endpoints that receive a signed JSON POST (HMAC-SHA256, `X-Sveltry-Signature`) on issue-lifecycle events (resolved / ignored / unresolved / assigned / unassigned), fired from the dashboard and the public API. SSRF-guarded delivery off the triggering mutation, an 8s timeout, a write-only secret shown once, admin-gated management, and per-attempt logging in `webhookDeliveries`. |
...
| Auto-resolve from commit messages | Done | A commit message that references an issue short id with a fix/close/resolve verb (`Fixes WEB-1A2B3C`) resolves that issue when the commits are uploaded, scoped to the commits' project and resolved-in-this-release (a later regression reopens it). No external provider call. |
...
| Slowest operations (cross-transaction) | Done | The Performance page aggregates spans across recent transactions, grouped by operation and description, ranked by total time spent (count / avg / p95 / total), with an operation-category filter. A recent-window sample (no columnar store yet). |
| N+1 detection | Done | The transaction detail page flags a potential N+1 when the same db / cache operation repeats many times in one transaction (a query in a loop), showing the operation, repeat count, and total time. |
| Trace explorer / span search | Done | A Span search page finds individual spans across recent transactions by operation or description, ranked by duration, each linking back to its transaction. The "Slowest operations" rows drill into it. A recent-window sample (no columnar store yet). |
| Performance issues | Done | A Performance issues page detects N+1 queries (a db/cache op repeated within one transaction), slow DB queries, and slow outbound HTTP calls across recent transactions, grouped by (type, operation, description) and ranked by total impact time, with a per-type filter and a drill-down to the worst sample transaction. Detected on read over the span data (a recent-window sample); not persisted as triageable issues. |
