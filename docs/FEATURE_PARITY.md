...
| Public REST API / API tokens | Partial | A v1 API authenticated by org API tokens (Bearer, `read` or `read+write` scope, managed on the settings page): `GET /projects`, `/releases`, `/members`, `/projects/<slug>/issues`, `/issues/<id>`, `/events/<eventId>`, `/issues/<id>/events`, and `POST /issues/<id>/{assign,resolve,ignore,unresolve}` (write scope). List endpoints support opaque cursor pagination (`?cursor`, `?limit`, `nextCursor`). Still missing vs Sentry: project-scoped event lookup, deploys/artifacts, and webhooks. |
